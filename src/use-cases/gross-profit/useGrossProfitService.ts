...
interface GrossProfitServiceState {
  data: GrossProfitProduct | null;
  isLoading: boolean;
  error: string | null;
}

export function useGrossProfitService(itemId: string | undefined, companyId: string | undefined, startDate: string, endDate: string) {
  const { session, isAuthenticated, isLoading: sessionLoading } = useSession();
  const [state, setState] = useState<GrossProfitServiceState>({
    data: null,
    isLoading: true,
    error: null,
  });

  const fetchService = useCallback(async (): Promise<GrossProfitProduct | null> => {
  ...
  const refetch = useCallback(() => {
    void (async () => {
      ...
      try {
        ...
        const message =
          err instanceof Error ? err.message : 'Error al cargar utilidad bruta del servicio';
